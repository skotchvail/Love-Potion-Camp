...
//    ledMapDump(2, 7); //set which strands you want to dump
    ...
    totalControlConsumer = new DefaultTotalControlConsumer();
    if (runConcurrent) {
      totalControlConsumer = new ConcurrentTotalControlConsumer(totalControlConsumer);
    }

    int status = totalControlConsumer.setupHardware(getNumStrands(), maxPixelsPerStrand, kUseBitBang);
    if (status != 0) {
  ...
  void drawToLeds() {
    ...
    totalControlConsumer.writeOneFrame(pixelData, theStrandMap);
  }

  void shutdown() {
    if (totalControlConsumer != null) {
